...
describe("KitchenTicketsService", () => {
  ...
  it("links ticket items to preparation tasks found after task creation", async () => {
    ...
    await service.createTicketsForAcceptedOrder(
      "order-1",
      "staff-1",
      tx as never,
    );
  ...
  it("creates visible KDS tickets without critical print or realtime side effects", async () => {
    ...
    const realtimeEventsService = {
      recordKitchenTicketCreated: jest
        .fn()
  ...
  it("syncs task ready compactly without realtime or full ticket hydration", async () => {
    const tx = {
      kitchenTicketItem: {
        findFirst: jest.fn().mockResolvedValue({
          id: "ticket-item-1",
    ...
    const result = await service.syncTicketsForTaskReady(
      "task-1",
      tx as never,
    ...
    expect(tx.kitchenTicket.findUnique).toHaveBeenCalledTimes(1);
    expect(findOneSpy).not.toHaveBeenCalled();
    expect(
      realtimeEventsService.recordKitchenTicketReady,
    ).not.toHaveBeenCalled();
  });
});
