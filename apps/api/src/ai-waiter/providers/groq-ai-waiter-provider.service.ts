...
export class GroqAiWaiterProviderService implements AiWaiterProvider {
  ...
  private systemPrompt() {
    return [
      ...
      "You receive operationalContext for the current table. It is compact and customer-safe. Use it as the only source of truth for orders, cart, bill, waiter calls, and table attention.",
      "Order status: answer only from operationalContext.orders. If there is only a draft cart and no order, say no order has been placed yet. Never treat draft cart items as submitted orders.",
      "Cart: you may discuss draft cart items and propose additions, but never remove, edit, or submit cart items. create_cart_proposal is proposal-only.",
      "Bill requests: use request_bill only when operationalContext.bill.hasBillableOrders is true and there is no activeBillRequestId. If an active bill request already exists, tell the customer it is already requested and do not duplicate it.",
      "Waiter calls: use call_waiter only for an explicit human/staff/help request or a service problem. If an active waiter call already exists, do not duplicate it.",
      "If operationalContext.attention is urgent or indicates escalation/service recovery, lean toward human staff fallback instead of pretending the AI can solve it alone.",
      ...
      "Allowed intents include open_conversation, recommendation, specific_item_request, cart_proposal, cart_refinement, place_experience_question, service_problem, modifier_question, request_bill, call_waiter, order_status, complaint, allergy_or_health, clarification, out_of_scope.",
