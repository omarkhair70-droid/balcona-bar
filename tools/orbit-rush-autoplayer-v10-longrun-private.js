(() => {
  ...
  /*
    Orbit Rush Autoplayer V10 — PRIVATE / LOCAL COPY ONLY
    =====================================================
  ...
  */
  ...
  if (BLOCKED_HOST_RE.test(host)) {
    console.error('[OrbitBot V10] Refusing to run on public 101-creations.com.');
    return;
  }

  if (!allowed) {
    console.error(
      '[OrbitBot V10] Host is not whitelisted:',
      host,
  ...
  if (!cv) {
    console.error('[OrbitBot V10] #canvas not found.');
    return;
  }
  ...
  if (!ctx) {
    console.error('[OrbitBot V10] 2D context unavailable.');
    return;
  }
  ...
  const cfg = {
    ...
    guardTriggerClearance: 24,
    guardReleaseClearance: 40,
    guardLatchMs: 260,
    guardReleaseSafeFrames: 5,
    guardChangePenalty: 0.020,
    guardMovePenalty: 0.010,
    guardEdgeCost: 30,
  ...
  function workerMain() {
    ...
    function reconstructTrajectory(best, maxSteps) {
      const reversed = [];
      let cur = best;

      while (cur && cur.parent) {
        reversed.push(cur.pointerTarget);
        cur = cur.parent;
      }

      reversed.reverse();

      return reversed.slice(0, maxSteps);
    }
  ...
  plannerWorker.onerror = err => {
    ...
    console.error('[OrbitBot V10 worker]', err);
  };
  ...
  function ageCompensatedWorkerTarget(now) {
    ...
    const ageMs = now - latestPlan.snapshotAt;

    if (ageMs > cfg.workerMaxAcceptedAgeMs) {
      return null;
    }
  ...
  function buildGuardPaths() {
    ...
    const hazards = [...hazardRefs].filter(z =>
      z.y <= player.y + 105 &&
      z.y +
        z.vy *
        fm.physicsRate *
  ...
  function candidateGuardTarget(gp, referenceTarget) {
    const rect = cv.getBoundingClientRect();
    ...
    for (let i = 0; i < cfg.guardCandidates; i++) {
      ...
      const hitCount = ev.hits.size;

      const legalShield =
        shieldActive() &&
        hitCount === 1;

      if (hitCount > 0 && !legalShield) {
        continue;
      }
      ...
      const score =
        ...
        (
          Number.isFinite(referenceTarget)
            ? Math.abs(x - referenceTarget) *
              cfg.guardChangePenalty
            : 0
        ) -
        edgePenalty -
        shieldPenalty;
  ...
  function arbitrateTarget(workerTarget, now) {
    ...
    const workerHitCount = workerEval.hits.size;
    ...
    if (guardLatched) {
      ...
      } else if (!Number.isFinite(guardTargetX)) {
  ...
  function renderHud() {
    ...
    hud.textContent =
      'ORBIT BOT V10 · LONG-RUN CANDIDATE\n' +
      'mode: ' + plannerMode +
      '\nguard latch: ' +
      (guardLatched ? 'YES' : 'NO') +
      '\nhazards: ' +
  ...
  function destroy() {
    ...
    console.log('[OrbitBot V10] destroyed.');
  }
  ...
  console.log(
    ...
    '\nV9 performance retained; stale-plan/guard conflict fixed.',
    '\nHost:', host || '(file://)',
    '\nF8 toggles.'
  );
})();