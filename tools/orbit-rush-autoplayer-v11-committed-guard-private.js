(() => {
  ...
  /*
    Orbit Rush Autoplayer V11 — LONG-RUN PRIVATE / LOCAL ONLY
    =====================================================
  ...
  */
  ...
  if (BLOCKED_HOST_RE.test(host)) {
    console.error('[OrbitBot V11] Refusing to run on public 101-creations.com.');
    return;
  }

  if (!allowed) {
    console.error(
      '[OrbitBot V11] Host is not whitelisted:',
      host,
  ...
  if (!cv) {
    console.error('[OrbitBot V11] #canvas not found.');
    return;
  }
  ...
  if (!ctx) {
    console.error('[OrbitBot V11] 2D context unavailable.');
    return;
  }
  ...
  const cfg = {
    ...
    guardTriggerClearance: 26,
    guardReleaseClearance: 42,
    guardLatchMs: 320,
    guardReleaseSafeFrames: 6,
    guardReplanEveryMs: 72,
    guardEmergencyClearance: 7,
    guardChangePenalty: 0.080,
    guardDirectionFlipPenalty: 160,
    guardMovePenalty: 0.009,
    guardEdgeCost: 30,
  ...
  plannerWorker.onerror = err => {
    ...
    console.error('[OrbitBot V11 worker]', err);
  };
  ...
  function candidateGuardTarget(
    ...
    for (let i = 0; i < cfg.guardCandidates; i++) {
      ...
      const direction =
        Math.abs(x - player.x) < 8
          ? 0
          : Math.sign(x - player.x);

      const directionFlipPenalty =
        lockedDirection !== 0 &&
        direction !== 0 &&
  ...
  function renderHud(force = false) {
    ...
    hud.textContent =
      'ORBIT BOT V11 · LONG-RUN CANDIDATE\n' +
      'mode: ' + plannerMode +
      '\nguard latch: ' +
      (guardLatched ? 'YES' : 'NO') +
      '\nhazards: ' +
  ...
  function destroy() {
    ...
    console.log('[OrbitBot V11] destroyed.');
  }
  ...
  console.log(
    ...
    '\nV10 trajectory retained; guard oscillation + main-thread churn fixed.',
    '\nHost:', host || '(file://)',
    '\nF8 toggles.'
  );
})();